...
export const memoryVectorStore = new OPSQLiteVectorStore({
  name: 'tarai',
...
// Convert memory to searchable string
export const memoryToString = (memory: {
  title: string;
  type: string;
  data?: string;
}) => {
  const parsed = memory.data ? JSON.parse(memory.data) : {};
  return `${memory.type}: ${memory.title}. ${parsed.desc || ''} ${parsed.tags || ''}`;
};

// Text splitter for long content
export const memorySplitter = new RecursiveCharacterTextSplitter({
  chunkSize: 500,
  chunkOverlap: 100,
});
