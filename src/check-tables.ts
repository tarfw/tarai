...
async function checkTables() {
    ...
    `);
		...
		if (tablesResult.rows.some((row: any) => row.name === "embeddings")) {
			...
			const schemaResult = await commerceDb.execute(
		...
		const productsResult = await commerceDb.execute(
			"SELECT COUNT(*) as count FROM products",
		);
		console.log(
			`📦 Products in database: ${(productsResult.rows[0] as any).count}`,
		);

		// Check if there are any embeddings
		const embeddingsResult = await commerceDb.execute(
			"SELECT COUNT(*) as count FROM embeddings",
		);
		console.log(
			`🧠 Embeddings in database: ${(embeddingsResult.rows[0] as any).count}`,
		);
	} catch (error) {
