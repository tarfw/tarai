...
export const orderProcessingWorkflow = createWorkflowChain({
  ...
  purpose: "Process customer orders with inventory validation, payment processing, and confirmation",
...
export const productRecommendationWorkflow = createWorkflowChain({
  ...
  purpose: "Generate personalized product recommendations based on user preferences and behavior",
  ...
  .andThen({
    ...
    execute: async ({ data }) => {
      ...
      if (data.preferences?.categories?.length) {
        filters.push(`category IN (${data.preferences.categories.map(c => `'${c}'`).join(',')})`);
      }
  ...
  .andThen({
    ...
    execute: async ({ data }) => {
      ...
      const products = await CommerceDB.searchProducts(data.searchQuery || '', undefined, 50);
